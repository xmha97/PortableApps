...
${SegmentPreExec}
	${IF} ${FileExists} "$EXEDIR\Data\.abdm"
		CreateDirectory "$EXEDIR\Data\.abdm"
		Rename "$EXEDIR\Data\.abdm" "$EXEDIR\App\ABDownloadManager\.abdm"
	${Else}
		CreateDirectory "$EXEDIR\App\ABDownloadManager\.abdm"
	${EndIf}
	...
	${If} $1 == "NotDone" ;Fallback to English
		StrCpy $1 "en_US"
	${EndIf}
	${If} ${FileExists} "$EXEDIR\App\ABDownloadManager\.abdm\config\appSettings.json"
		;Determine if run from the PortableApps.com Menu
		ReadEnvStr $0 "PortableApps.comLanguageCode_INTERNAL"
		${IfNot} $0 == ""
			nsExec::ExecToStack '$\"$EXEDIR\App\Bin\jq.exe$\" $\".language = \$\"$1\$\"$\" $\"$EXEDIR\App\ABDownloadManager\.abdm\config\appSettings.json$\"'
			Pop $6
			${If} $6 == "0"
				Pop $7
				FileOpen $0 "$EXEDIR\App\ABDownloadManager\.abdm\config\appSettings.json" w ;Opens a Empty File and fills it
				FileWrite $0 $7
				FileClose $0 ;Closes the filled file
			${EndIf}
		${EndIf}
	${Else}
		CreateDirectory "$EXEDIR\App\ABDownloadManager\.abdm\config"
		FileOpen $2 "$EXEDIR\App\ABDownloadManager\.abdm\config\appSettings.json" w ;Opens a Empty File and fills it
		FileWrite $2 "{$\n"
...
${SegmentPostPrimary}
	CreateDirectory "$EXEDIR\Data\.abdm"
	Rename "$EXEDIR\App\ABDownloadManager\.abdm" "$EXEDIR\Data\.abdm"
!macroend
