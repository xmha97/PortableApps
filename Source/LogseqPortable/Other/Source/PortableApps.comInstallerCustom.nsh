!macro CustomCodePostInstall
	Rename "$INSTDIR\App\lib\net45" "$INSTDIR\App\Logseq"
	RMDir /r "$INSTDIR\App\lib"
	...
	${If} $1 == "NotDone" ;Fallback to English
		StrCpy $1 "en-US"
	${EndIf}
	CreateDirectory "$INSTDIR\Data\LogseqAppData\Local Storage\leveldb"
	nsExec::Exec `"$INSTDIR\App\Bin\leveldb.exe" --dbpath="$INSTDIR\Data\LogseqAppData\Local Storage\leveldb" init`
	nsExec::Exec `"$INSTDIR\App\Bin\leveldb.exe" --dbpath="$INSTDIR\Data\LogseqAppData\Local Storage\leveldb" put "_file://\0\x01preferred-language" "\x01\"$1\""`
!macroend
